...
const smallerSphereGeometry = new THREE.SphereGeometry(1, 32, 32);
...
scene.remove(cube);
scene.remove(sphere);
scene.remove(smallerSphere);
scene.add(resultMesh);

// resultMesh.scale.x = 2;
// resultMesh.scale.y = 2;
// resultMesh.scale.z = 2;
const resultCSG = CSG.fromMesh(resultMesh);
const cubeGroupCSG = CSG.fromMesh(cubeCrystalClear);
const rdSubtractedCSG = cubeGroupCSG.subtract(resultCSG);
const ndMesh = CSG.toMesh(rdSubtractedCSG, new THREE.Matrix4, crystalClearMaterial);
